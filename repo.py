...
class Repository:
    ...
    def create_type(self, characteristics: list, type_name: str):
        try:
            id_characteristics = []
            ...
            for x in characteristics:
                ...
                self.cursor.execute(sql)
                self.conn.commit()
                sql = 'select id_drone_characteristics from drone_characteristic where name like \'%s\''%x
                self.cursor.execute(sql)
                id_characteristics.append(self.cursor.fetchone())
            for x in id_characteristics:
                sql = 'insert into drone_characteristics_type(id_drone_type, id_drone_characteristics) values(\'%s\'),(\'%s\')'%type%x
                self.cursor.execute(sql)
                self.conn.commit()
        except Exception as e:
            raise Exception(e)
...
print(print(repo.get_characteristics_template(3)))